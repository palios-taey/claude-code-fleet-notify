...
| Stop engine | Integrated, not owned. Stop hooks call the orchestrator `/stop-decision` endpoint when configured, then set Redis idle state and notify the supervisor. | `hooks/_shared.py:fetch_stop_decision`; `hooks/stop_idle.py`; Redis key `${NOTIFY_KEY_PREFIX:-taey}:<node>:idle`. |
...
| Wake packet | Integrated, not owned. Post-tool/user-prompt hooks fetch the orchestrator wake packet and append it to delivered context when enabled upstream. | `hooks/_shared.py:_fetch_wake_packet`; set `ORCH_API_BASE` and `ORCH_WAKE_PACKET_ENABLED=1` on the orchestrator API. |
...
| Daemon | Live by default when started. One local daemon watches Redis inboxes and tmux idle state, then pointer-injects pending messages into idle sessions. | `bash scripts/start_notify_daemons.sh start`; `bash scripts/start_notify_daemons.sh status`; `/tmp/notify-daemon.log`. |
