...
The installer also wires a second pre-tool guard hook for Claude Code, codex, and gemini. The guard reads a live-path registry from `CF_LIVE_PATH_REGISTRY` (or `ORCH_LIVE_PATH_REGISTRY`), then falls back to the committed registry at `$HOME/the-conductor/config/live_path_registry.json`; it never copies registry contents into this repo. See [config/live_path_registry.example.json](config/live_path_registry.example.json) for the expected shape. With a valid registry, destructive git/filesystem commands targeting a registered live checkout are denied and the operator must cut a worktree first. Registered worktree-relative targets remain allowed, while explicit absolute live-checkout targets are still denied. If registry/configuration, command parsing, or internal guard state is unavailable, read-only calls remain available and guarded mutating shell calls fail closed. One defect per node and failure class is routed to the node's supervisor (or the node itself for a top-level session) with a 15-minute dedupe TTL; ordinary read-only calls do not receive repeated warning context.
